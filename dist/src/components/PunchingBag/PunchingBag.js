...
const PunchingBag = () => {
    ...
    const punchingBagRef = useRef(null);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < positions.count; i++) {
            ...
            if (y > 2) {
                ...
                positions.setXYZ(i, x * (1 - factor * 0.3), y, z * (1 - factor * 0.3));
            }
        }
        ...
        const audience = []; // Replace with actual audience creation
        ...
        const handleClick = (event) => {
            if (!punchingBagRef.current || !cameraRef.current)
                return;
    ...
    return (<div ref={containerRef} className="w-full h-screen" style={{ cursor: 'pointer' }}>
			<Button onClick={() => navigate('/games')} className="absolute top-4 left-4 bg-black/50 hover:bg-black/70 text-white">
				Back to Games
			</Button>
			...
			<input type="file" accept="image/*" className="absolute bottom-4 left-1/2 transform -translate-x-1/2 text-white" onChange={(e) => {
