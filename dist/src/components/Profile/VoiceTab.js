...
import { Play, Pause, Share2, Trash2, Search, Calendar, } from 'lucide-react';
    ...
    return (<div className="space-y-6">
      <div className="flex items-center gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search voice recordings..." className="pl-10"/>
        </div>
      ...
      <div className="space-y-4">
        {voiceEntries.map((entry) => (<Card key={entry.id} className="hover:shadow-md transition-shadow duration-300">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="icon" onClick={() => handlePlay(entry.content)}>
                    {isPlaying ? (<Pause className="w-4 h-4"/>) : (<Play className="w-4 h-4"/>)}
                  </Button>
                  <Button variant="outline" size="icon" onClick={() => handleShare(entry.id)}>
        ...
        <DialogContent>
          ...
          {selectedEntry && (<div className="space-y-4">
              <div className="bg-muted p-4 rounded-lg">
                <Button variant="outline" size="icon" onClick={() => handlePlay(voiceEntries.find(e => e.id === selectedEntry)?.content)} className="mx-auto block">
                  {isPlaying ? (<Pause className="w-6 h-6"/>) : (<Play className="w-6 h-6"/>)}
                </Button>
              </div>
              
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => handleShare(selectedEntry)}>
                  <Share2 className="w-4 h-4 mr-2"/>
                  Share
                </Button>
                <Button variant="destructive" onClick={() => handleDelete(selectedEntry)}>
