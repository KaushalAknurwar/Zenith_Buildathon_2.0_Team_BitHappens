...
import { Share2, Trash2, Search, Calendar } from 'lucide-react';
...
const DrawingsTab = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {drawingEntries.map((entry) => (<Card key={entry.id} className="group hover:shadow-lg transition-shadow duration-300" onClick={() => setSelectedDrawing(entry.id)}>
            <CardContent className="p-4">
              <div className="relative aspect-square rounded-lg overflow-hidden">
                <img src={entry.content} alt={entry.title || 'Drawing'} className="w-full h-full object-cover"/>
      ...
      <Dialog open={!!selectedDrawing} onOpenChange={() => setSelectedDrawing(null)}>
        <DialogContent className="max-w-3xl">
          ...
          {selectedDrawing && (<div className="space-y-4">
              <img src={entries.find(e => e.id === selectedDrawing)?.content} alt="Drawing" className="w-full rounded-lg"/>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => handleShare(selectedDrawing)}>
                  <Share2 className="w-4 h-4 mr-2"/>
                  Share
                </Button>
                <Button variant="destructive" onClick={() => handleDelete(selectedDrawing)}>
