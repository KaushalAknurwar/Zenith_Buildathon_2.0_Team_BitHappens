...
const TestimonialCard = ({ position, testimonial, cardSize }) => {
    const isCenter = position === 0;
    return (<div className={cn("absolute left-1/2 top-1/2 border-2 p-8 transition-all duration-500 ease-in-out backdrop-blur-md", isCenter
            ? "z-10 bg-white/10 text-white border-white/20"
            : "z-0 bg-white/5 text-white/80 border-white/10")} style={{
            width: cardSize,
            height: cardSize,
      ...
      <span className="absolute block origin-top-right rotate-45 bg-white/10" style={{
            right: -2,
            top: 48,
            width: SQRT_5000,
            height: 2
        }}/>
      <img src={testimonial.imgSrc} alt={`${testimonial.by.split(',')[0]}`} className="mb-4 h-14 w-12 bg-white/5 object-cover object-top" style={{
            boxShadow: "3px 3px 0px rgba(0, 0, 0, 0.2)"
        }}/>
      <h3 className={cn("text-base sm:text-xl font-medium", isCenter ? "text-white" : "text-white/80")}>
        "{testimonial.testimonial}"
      </h3>
      <p className={cn("absolute bottom-8 left-8 right-8 mt-2 text-sm italic", isCenter ? "text-white/80" : "text-white/60")}>
        - {testimonial.by}
      </p>
    </div>);
};
export const StaggerTestimonials = () => {
    ...
    const moveTestimonialsLeft = () => {
        setTestimonialsList(prevList => {
            ...
            if (!firstItem)
                return prevList; // Should not happen with a non-empty array
    ...
    return (<div className="relative w-full overflow-hidden bg-[#1A1A2E]" style={{ height: 600 }}>
      {testimonialsList.map((testimonial, index) => {
            const position = index - Math.floor(testimonialsList.length / 2);
            return (<TestimonialCard key={testimonial.tempId} testimonial={testimonial} position={position} cardSize={cardSize}/>);
        })}
    </div>);
};
