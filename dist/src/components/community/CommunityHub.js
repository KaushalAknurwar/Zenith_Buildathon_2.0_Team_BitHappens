...
import { BookOpen, Video, Headphones, Award, Search, BookMarked, PlayCircle, FileText, ArrowRight, Star } from 'lucide-react';
...
const CommunityHub = () => {
    ...
    const handleResourceComplete = (points) => {
    ...
    return (<div className="space-y-6 p-4 sm:p-6 bg-black/40 bg-gradient-to-br from-[#8B5CF6]/5 to-[#D946EF]/5 backdrop-blur-md rounded-xl border-white/20">
      {/* Header */}
      ...
      <div className="relative max-w-xl mx-auto w-full">
        ...
        <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search resources..." className="pl-10 bg-black/20 border-white/20 text-white placeholder:text-white/50"/>
      </div>

      {/* Content Tabs */}
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="w-full bg-black/20 border-white/20 p-1 flex flex-wrap sm:flex-nowrap">
          <TabsTrigger value="all" className="flex-1 min-w-[100px] data-[state=active]:bg-white/10 data-[state=active]:text-white">
            All
          </TabsTrigger>
          <TabsTrigger value="articles" className="flex-1 min-w-[100px] data-[state=active]:bg-white/10 data-[state=active]:text-white">
            <FileText className="w-4 h-4 mr-2 hidden sm:inline-block"/>
            Articles
          </TabsTrigger>
          <TabsTrigger value="videos" className="flex-1 min-w-[100px] data-[state=active]:bg-white/10 data-[state=active]:text-white">
            <Video className="w-4 h-4 mr-2 hidden sm:inline-block"/>
            Videos
          </TabsTrigger>
          <TabsTrigger value="audio" className="flex-1 min-w-[100px] data-[state=active]:bg-white/10 data-[state=active]:text-white">
        ...
        <TabsContent value="all" className="mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {resources.map((resource) => (<motion.div key={resource.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <Card className="p-4 sm:p-6 bg-black/40 backdrop-blur-md border-white/20 hover:border-[#8B5CF6]/50 transition-all duration-300">
                  <div className="flex flex-col h-full">
                    ...
                    <div className="mt-6 pt-4 border-t border-white/10">
                      <Button variant="ghost" className="w-full text-white hover:bg-white/10" onClick={() => handleResourceComplete(10)}>
                        Start Learning
        ...
        <TabsContent value="articles">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {resources.filter(r => r.type === 'article').map((resource) => (<motion.div key={resource.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <Card className="p-4 sm:p-6 bg-black/40 backdrop-blur-md border-white/20 hover:border-[#8B5CF6]/50 transition-all duration-300">
                  <div className="flex flex-col h-full">
                    <div className="flex flex-col sm:flex-row items-start gap-4">
                      <div className="flex-shrink-0">
        ...
        <TabsContent value="videos">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {resources.filter(r => r.type === 'video').map((resource) => (<motion.div key={resource.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <Card className="p-4 sm:p-6 bg-black/40 backdrop-blur-md border-white/20 hover:border-[#8B5CF6]/50 transition-all duration-300">
                  <div className="flex flex-col h-full">
                    <div className="flex flex-col sm:flex-row items-start gap-4">
                      <div className="flex-shrink-0">
        ...
        <TabsContent value="audio">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {resources.filter(r => r.type === 'audio').map((resource) => (<motion.div key={resource.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <Card className="p-4 sm:p-6 bg-black/40 backdrop-blur-md border-white/20 hover:border-[#8B5CF6]/50 transition-all duration-300">
                  <div className="flex flex-col h-full">
                    <div className="flex flex-col sm:flex-row items-start gap-4">
                      <div className="flex-shrink-0">
