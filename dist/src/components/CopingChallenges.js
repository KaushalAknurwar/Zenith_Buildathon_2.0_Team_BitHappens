    ...
    return (<div className="space-y-8 p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-8">
        {challenges.map(challenge => {
            ...
            return (<Card key={challenge.id} className={`p-8 transition-all duration-300 hover:shadow-lg transform hover:-translate-y-1 ${challenge.completed ? 'opacity-75' : ''} bg-zinc-900/90 backdrop-blur-sm border-2 border-zinc-800 text-white`}>
              <div className="flex flex-col h-full space-y-6">
                ...
                <Button className={`w-full rounded-xl h-12 text-base font-semibold transition-all duration-300 ${challenge.completed
                    ? 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                    : 'bg-duo-purple hover:bg-duo-purple/90 text-white'}`} onClick={() => completeChallenge(challenge.id)} disabled={challenge.completed}>
                  {challenge.completed ? (<div className="flex items-center justify-center gap-2">
                      <Check className="w-5 h-5"/>
                      Completed
                    </div>) : ('Complete Challenge')}
                </Button>
              </div>
            </Card>);
        })}
