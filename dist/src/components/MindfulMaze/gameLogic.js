...
const ensureMinimumPathWidth = (maze) => {
...
export const generateMaze = (difficulty) => {
    const wallDensity = {
        easy: 0.25,
        medium: 0.35,
        hard: 0.45
    ...
    do {
        newMaze = Array(MAZE_SIZE).fill(0).map(() => Array(MAZE_SIZE).fill(0).map(() => Math.random() > wallDensity ? 0 : 1));
...
export const generateCollectibles = (maze) => {
    if (!maze || maze.length === 0)
        return [];
    const items = [];
    const types = ['star', 'crystal', 'heart'];
...
export const calculatePoints = (collectibleType) => {
    const points = {
        star: 10,
        crystal: 15,
        heart: 20
