...
const VideoEntry = () => {
    ...
    const handleEmotionToggle = (emotionId) => {
        setSelectedEmotions(prev => prev.includes(emotionId)
    ...
    return (<div className="space-y-6">
      ...
      {videoBlob && (<>
          <EmotionTagger selectedEmotions={selectedEmotions} onEmotionToggle={handleEmotionToggle}/>

          <Card className="p-4 space-y-4">
            <h3 className="text-lg font-semibold">Reflect on your entry</h3>
            <Textarea value={reflection} onChange={(e) => setReflection(e.target.value)} placeholder="What thoughts or insights would you like to add to this entry?" className="min-h-[100px]"/>
          </Card>

          <div className="flex items-center justify-between">
            <Button variant="outline" onClick={() => setIsPrivate(!isPrivate)} className="flex items-center space-x-2">
              {isPrivate ? (<>
