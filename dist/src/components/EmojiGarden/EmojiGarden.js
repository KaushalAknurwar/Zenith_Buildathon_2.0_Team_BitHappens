...
const EmojiGarden = () => {
    ...
    const [cells, setCells] = useState(Array(25).fill(null).map((_, i) => ({
        id: `cell-${i}`,
        emoji: null,
        emotion: '',
        description: '',
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gradient-to-br from-[#0A0F1C] to-[#1A1F3C] px-4 py-6 sm:p-6 text-white">
      <motion.div variants={container} initial="hidden" animate="show" className="max-w-6xl mx-auto space-y-8">
        <motion.div variants={item} className="text-center mb-12 animate-fade-in relative">
        ...
        <motion.div variants={item} className="bg-white/5 backdrop-blur-lg rounded-2xl shadow-lg border border-white/10 p-6 space-y-4">
        ...
        <motion.div variants={item} className="bg-white/5 backdrop-blur-lg rounded-2xl shadow-lg border border-white/10 p-6 space-y-4">
          ...
          <GardenGrid cells={cells} onCellClick={handleCellClick} onWater={handleWater}/>
        </motion.div>
      </motion.div>
    </motion.div>);
};
