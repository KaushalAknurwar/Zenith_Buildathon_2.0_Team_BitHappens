...
export const AshaChatbot = () => {
    ...
    return (<>
      ...
      <motion.button className="fixed bottom-8 right-8 z-50 rounded-full bg-gradient-to-r from-[#8B5CF6] to-[#D946EF] p-4 text-white shadow-lg hover:shadow-xl transition-shadow duration-300" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setIsOpen(true)}>
      ...
      <AnimatePresence>
        {isOpen && (<>
            {/* Backdrop */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[99]" onClick={() => setIsOpen(false)}/>

            {/* Chat Window */}
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="fixed inset-0 flex items-center justify-center z-[100] p-4">
                ...
                <div className="flex-shrink-0 flex items-center gap-2 p-4 border-b border-white/20 bg-black/20">
                  ...
                  <Button size="icon" variant="ghost" className="ml-auto text-white/60 hover:text-white" onClick={() => setIsOpen(false)}>
                    <X className="w-4 h-4"/>
                  </Button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {messages.map((message) => (<div key={message.id} className={cn("flex gap-3 max-w-[80%]", message.sender === 'user' ? "ml-auto flex-row-reverse" : "")}>
                    {message.sender === 'bot' && (<Bot className="w-8 h-8 text-[#D946EF] flex-shrink-0"/>)}
                    <div className={cn("rounded-2xl p-3", message.sender === 'user'
                    ? "bg-gradient-to-r from-[#8B5CF6] to-[#D946EF] text-white"
                    : "bg-white/10 text-white")}>
                    {message.content}
                    </div>
                  </div>))}
                  {isTyping && (<div className="flex gap-3 max-w-[80%]">
                      <Bot className="w-8 h-8 text-[#D946EF]"/>
                ...
                <div className="flex-shrink-0 mt-auto">
                  ...
                  <div className="flex gap-2">
                    <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your message..." className="flex-1 bg-white/10 border-white/20 text-white placeholder:text-white/40"/>
                    <Button type="submit" size="icon" className="bg-gradient-to-r from-[#8B5CF6] to-[#D946EF] text-white hover:opacity-90" disabled={!input.trim() || isTyping}>
