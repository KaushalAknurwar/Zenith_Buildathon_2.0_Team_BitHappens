...
const TherapistListing = () => {
    ...
    const filterTherapists = (search, specialization) => {
        ...
        if (specialization) {
            filtered = filtered.filter(t => t.specialization.includes(specialization));
        }
        setTherapists(filtered);
    };
    return (<div className="space-y-6">
			<div className="flex gap-4 mb-6">
				<Input placeholder="Search therapists..." value={searchTerm} onChange={handleSearch} className="max-w-sm bg-white/5 backdrop-blur-lg border border-white/10 text-white placeholder:text-white/60"/>
				<select className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-md p-2 text-white outline-none" value={selectedSpecialization} onChange={(e) => {
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{therapists.map((therapist) => (<Card key={therapist.id} className="bg-white/5 backdrop-blur-lg border border-white/10 p-4">
						<div className="flex items-start gap-4">
							<img src={therapist.image} alt={therapist.name} className="w-20 h-20 rounded-full border-2 border-[#8B5CF6]/30"/>
							<div>
								<h3 className="font-semibold text-white">{therapist.name}</h3>
								<div className="flex flex-wrap gap-1 mt-2">
									{therapist.specialization.map((spec) => (<Badge key={spec} variant="secondary" className="bg-[#8B5CF6]/20 text-white border border-[#8B5CF6]/30">
											{spec}
										</Badge>))}
								</div>
								...
								<Dialog>
									...
									<DialogContent className="bg-[#1A1F3C] border border-white/10 text-white">
										...
										<div className="space-y-4">
											...
											<Calendar mode="single" selected={selectedDate} onSelect={setSelectedDate} className="rounded-md border border-white/10 bg-white/5 text-white" classNames={{
                ...
                caption: "text-white"
											...
											<div className="grid grid-cols-3 gap-2">
												{["9:00 AM", "10:00 AM", "11:00 AM"].map((time) => (<Button key={time} variant="outline" className="w-full bg-white/5 border-white/10 text-white hover:bg-white/20">
														{time}
													</Button>))}
											</div>
											{showPaymentAlert && (<Alert className="bg-white/5 border border-white/10">
													<AlertCircle className="h-4 w-4 text-white"/>
													<AlertDescription className="text-white">
														Processing payment... Please wait.
													</AlertDescription>
												</Alert>)}
											<Button className="w-full bg-gradient-to-r from-[#8B5CF6] to-[#D946EF] text-white hover:opacity-90" onClick={handleBooking}>
