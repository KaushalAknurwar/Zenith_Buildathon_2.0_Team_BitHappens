...
const EmergencyBooking = () => {
    ...
    return (<div className="space-y-6">
			...
			{emergencyContacts.map(contact => (<Card key={contact.number} className="p-6 bg-red-500/10 backdrop-blur-lg border border-red-500/20 hover:bg-red-500/20 transition-all duration-300">
			...
			<form onSubmit={handleEmergencyBooking} className="space-y-4">
				<div>
					...
					<Input id="name" value={formData.name} onChange={handleChange} required className="bg-white/5 backdrop-blur-lg border border-white/10 text-white placeholder:text-white/60"/>
				</div>

				<div>
					<Label htmlFor="phone" className="text-white">Phone Number</Label>
					<Input id="phone" type="tel" value={formData.phone} onChange={handleChange} required className="bg-white/5 backdrop-blur-lg border border-white/10 text-white placeholder:text-white/60"/>
				</div>

				<div>
					<Label htmlFor="situation" className="text-white">Brief Description of Situation</Label>
					<Textarea id="situation" value={formData.situation} onChange={handleChange} required className="bg-white/5 backdrop-blur-lg border border-white/10 text-white placeholder:text-white/60 min-h-[100px]"/>
				</div>

				<Button type="submit" className="w-full bg-gradient-to-r from-red-500 to-red-600 text-white hover:opacity-90 transition-opacity" disabled={loading}>
					{loading ? (<>
