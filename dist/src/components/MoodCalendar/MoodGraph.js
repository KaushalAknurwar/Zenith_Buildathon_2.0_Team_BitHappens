...
const MoodGraph = ({ entries, view }) => {
    const getMoodData = () => {
        ...
        switch (view) {
            case 'day':
                ...
                data = hours.map(hour => {
                    const entry = entries.find(e => new Date(e.created_at).getHours() === hour.getHours() &&
                        format(new Date(e.created_at), 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd'));
                    return entry ? MOOD_VALUES[entry.mood] : 0;
                });
                break;
            case 'week':
                ...
                data = days.map(day => {
                    const dayEntries = entries.filter(e => format(new Date(e.created_at), 'yyyy-MM-dd') === format(day, 'yyyy-MM-dd'));
            ...
            case 'month':
                ...
                data = weeks.map(week => {
                    ...
                    if (weekEntries.length === 0)
                        return 0;
                    const avgMood = weekEntries.reduce((sum, entry) => sum + MOOD_VALUES[entry.mood], 0) / weekEntries.length;
                    return avgMood;
                });
                break;
        }
    ...
    return (<div className="p-4 bg-black/40 rounded-xl border border-white/20">
